...
아래 순서로 탐색한다. 함수명은 앞 섹션 grep 결과에서 확정된 것을 그대로 사용한다.
...
**Step 2: 전면형 광고 실제 진입점 파악**

PORTING_VOCAB.md `광고 전면 진입점` 행에서 `{INTERSTITIAL_ENTRY}` 값을 읽는다.  
→ 이 값이 전면 광고 진입점 함수명 (예: `ShowInterstitialAD`, `ShowInterstitial`, `ShowFullAd` 등)  
→ `없음`이면 전면 광고 미사용 — 기본 규칙 1 "동시 노출 경로 없음"으로 즉시 확정

**Step 3: 두 진입점이 동일 함수/콜백 내에서 공존하는지 확인**

```bash
# INTERSTITIAL_ENTRIES의 각 함수명으로 grep — 보상형 완료 콜백 파일 내에서 검색
grep -n "{INTERSTITIAL_ENTRY}" {AD_MANAGER} | grep -v "//"
grep -rn "{INTERSTITIAL_ENTRY}" {SCRIPTS_PATH} --include="*.cs" | grep -v "//"
```

히트한 라인이 보상형 완료 콜백(`onRewardResult`, `onRewardComplete`, `rewardSuccess` 등) 내부이면 → 동시 노출 경로 존재.

**Step 4: 보상형 완료 콜백 내부에서 전면 광고 호출 여부 직접 확인**

Step 3에서 히트가 없더라도, 보상형 완료 콜백 함수를 AD_MANAGER에서 Read해 블록 내부를 직접 확인한다.

**판정 기준:**
- 동일 콜백/함수 내에 두 진입점이 **모두 존재** → "동시 노출 불가 (보상형 완료 후 전면 억제)" 또는 "동시 노출 가능 (우선순위: 보상형 우선)" 중 코드 조건에 따라 확정
- 두 진입점이 **서로 다른 콜백/경로**에만 존재 → "동시 노출 경로 없음 (분리 호출)"
