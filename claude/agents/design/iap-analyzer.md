...
> **0건 처리 (공통 — 거짓 음성 방지)**: 1차 grep이 0건이어도 곧바로 `없음`·`미사용`으로 단정하지 않는다 — 좁은 키워드 grep은 자체 네이밍·한글 클래스명·문자열 조립(`"prod_" + id`)으로 만든 PID를 놓칠 수 있다. 0건이면 그 항목의 도메인 어휘(한글·동의어 포함)로 1회 더 넓게 재탐색한다(최대 5개 파일, 1단계). 넓힌 결과도 0건이면 `없음`(확신), 후보는 나왔으나 Read로 확정 못 하면 `확인 필요`로 기록한다 — **"없음" 임의 단정 금지.** 본문의 "없으면 없음"·"미사용 확정" 등은 모두 이 절차를 거친 뒤의 최종 판정을 뜻한다.
