...
**측정 여부는 포터가 임의로 정하지 않는다.** `pureweb-checklist.md` `## 확인 필요`에서 `[사람 준비] 사전 빌드 baseline 측정` 항목을 확인한다(h5-port STEP 3-A가 미리 수집):
- `측정함` → 아래 빌드를 실행해 기준 용량을 기록한다.
- `생략` → 스킵하고 작업 순서로 진행한다.
- **항목 자체가 없음**(오케스트레이터 없이 직접 호출된 경우 — 물어볼 주체가 없음) → 스킵하되 `## 확인 필요`에 `- [ ] [사람 준비] 사전 빌드 baseline 측정: 미정(직접 호출 — 필요 시 수동 실행)` 기록 후 진행한다.
...
> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/pureweb-patterns.md` → "0. SDK 초기화"
...
> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/pureweb-patterns.md` → "1-A. 리뷰 팝업 제거"
...
> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/pureweb-patterns.md` → "2. SafeArea 제거"
...
> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/pureweb-patterns.md` → "3. Screen.fullScreen / SetResolution 방지"
...
> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/pureweb-patterns.md` → "3-B. 외부 네트워크 요청 차단"
...
> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/pureweb-patterns.md` → "5. 광고 즉시 지급"
...
> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/pureweb-patterns.md` → "6. IAP 즉시 지급"
...
> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/pureweb-patterns.md` → "7-0. 사전 체크 — MonoBehaviour 씬/프리팹 첨부 여부"
...
> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/pureweb-patterns.md` → "7-3. C# 코드 가드"
...
> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/pureweb-patterns.md` → "7-5. WebGL 비호환 서비스 클래스 — 메서드 레벨 분기"
...
| 상황 | 처리 |
|---|---|
| 암호화 있음 | **제거한다(고정 정책 — 대안 없음).** `{LOCAL_SAVE_METHOD}`는 platform-porter 등 다른 WebGL 플랫폼도 그대로 재사용하는 공유 함수라(7-A 참조), 여기서 안 지우면 이후 모든 WebGL 플랫폼이 계속 암호화된 채로 남는다. 암호화 로직을 `#if !UNITY_WEBGL`로 격리하고 WebGL 분기에서 제거, 아래 Base64 래핑만 남긴다. |
| Base64만 있음(암호화 없음) | 기존 `{LOCAL_SAVE_METHOD}`가 이미 인코딩 — 아래에서 그대로 호출만 하면 됨 |
...
> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/pureweb-patterns.md` → "8. 서버 저장 차단 / LocalStorage 검증"
...
> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/pureweb-patterns.md` → "8-A. 저장 키 분리"
...
> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/pureweb-patterns.md` → "8-B. Base64 인코딩 래핑"
...
> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/pureweb-patterns.md` → "10. CheatConsole.prefab 씬 추가"
