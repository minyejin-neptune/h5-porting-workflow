...
```bash
echo "=== SafeArea (태스크 2) ===" && \
grep -rln "SafeArea\|safeArea\|GetSafeArea" Assets/Scripts --include="*.cs" 2>/dev/null | grep -v HyperLane

echo "=== Screen.fullScreen (태스크 3) ===" && \
grep -rln "Screen\.SetResolution\|Screen\.fullScreen" Assets/Scripts --include="*.cs" 2>/dev/null | grep -v HyperLane

echo "=== 광고 즉시지급 (태스크 5) ===" && \
# PORTING_VOCAB.md {AD_REWARDED_METHOD} 값으로 grep
grep -rln "{AD_REWARDED_METHOD}" Assets/Scripts --include="*.cs" 2>/dev/null | grep -v HyperLane

echo "=== IAP 즉시지급 (태스크 6) ===" && \
# PORTING_VOCAB.md {IAP_METHOD} 값으로 grep. IStoreListener는 SDK 인터페이스로 고정 검색
grep -rln "{IAP_METHOD}\|IStoreListener" Assets/Scripts --include="*.cs" 2>/dev/null | grep -v HyperLane

echo "=== 서버 저장 차단 (태스크 8) ===" && \
grep -rln "Backend\.\|SaveCloud\|ServerSave\|CloudSave\|File\.Read\|File\.Write\|StreamWriter\|StreamReader" Assets/Scripts --include="*.cs" 2>/dev/null | grep -v HyperLane
```
...
```bash
grep -rln "SafeArea\|safeArea\|GetSafeArea\|SafeAreaInsets" Assets/Scripts --include="*.cs" 2>/dev/null | grep -v HyperLane
```
...
```bash
grep -rn "Screen.SetResolution\|Screen.fullScreen" Assets/Scripts --include="*.cs" 2>/dev/null | grep -v HyperLane
```
...
grep -rn "Application\.OpenURL" Assets/Scripts --include="*.cs" 2>/dev/null \
  | grep -v "UNITY_WEBGL\|HyperLane\|//"
```
...
grep -rln "WEBGL_TOSS" Assets/Scripts --include="*.cs" 2>/dev/null \
...
grep -rn "{AD_REWARDED_METHOD}" Assets/Scripts --include="*.cs" 2>/dev/null | grep -v HyperLane
```
...
grep -rn "{AD_REWARDED_METHOD}" Assets/Scripts --include="*.cs" 2>/dev/null \
...
grep -rn "{IAP_METHOD}\|IStoreListener" Assets/Scripts --include="*.cs" 2>/dev/null | grep -v HyperLane
```
...
grep -rn "{IAP_METHOD}" Assets/Scripts --include="*.cs" 2>/dev/null \
  | grep -v "WEBGL_PUREWEB\|//"
```
...
grep -rln "using {SDK네임스페이스}" Assets/Scripts --include="*.cs" 2>/dev/null \
...
grep -rln "TcpClient\|System\.Threading\.Thread\b\|NetworkStream\|System\.Net\.Sockets" \
  Assets/Scripts --include="*.cs" 2>/dev/null | grep -v HyperLane
```
...
```bash
# System.IO 직접 사용
grep -rn "File\.Read\|File\.Write\|StreamWriter\|StreamReader" Assets/Scripts --include="*.cs" 2>/dev/null \
  | grep -v "UNITY_WEBGL\|//"

# 서버 저장 API 호출
grep -rn "Backend\.\|SaveCloud\|ServerSave\|CloudSave" Assets/Scripts --include="*.cs" 2>/dev/null \
  | grep -v "UNITY_WEBGL\|//"
```
...
grep -rn "using Firebase\|using AppsFlyer\|using Facebook\|using Airbridge" \
  Assets/Scripts --include="*.cs" 2>/dev/null | grep -v "UNITY_WEBGL\|//"

# 전처리문 — WEBGL_PUREWEB 단독 사용 감지 (결과 있으면 처리 필요)
grep -rn "#if WEBGL_PUREWEB\|#elif WEBGL_PUREWEB" Assets/Scripts --include="*.cs" 2>/dev/null \
  | grep -v "UNITY_WEBGL\|//"
...
grep -rn "{SAVE_METHOD}\|Backend\." Assets/Scripts --include="*.cs" 2>/dev/null \
  | grep -v "UNITY_WEBGL\|//"
...
grep -rn "RegisterCheats" Assets/Scripts --include="*.cs" 2>/dev/null | grep -v "//"
```
