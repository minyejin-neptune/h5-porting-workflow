...
| `Reset Local+Server (LIVE)` | `{키}_LIVE` | **하지 않음** — 설명(description) 문자열에 "Reset local data only. LIVE server data is NOT reset (production safety)." 명시만 하고 실제 `SetUserData` 호출은 코드에 넣지 않는다 |
...
PORTING_VOCAB.md `저장 인코딩` 행 + `{SAVE_METHOD}` 파일을 Read해서 직렬화 패턴을 파악한다. `Reset Local+Server (DEV)`만 실제 `SetUserData` 호출을 포함하므로(LIVE는 서버 초기화 자체를 안 함 — 위 표 참조), 빈 데이터 직렬화는 **검수 없이 활성화하지 않는다** → 결정 필요 라우팅(서버 초기화 빈 데이터 직렬화 검수 — 파악한 생성 예시 첨부). 검수 전까지 `Reset Local+Server (DEV)`는 `// TODO: 빈 데이터 직렬화 검수 후 활성화` 주석과 함께 비활성 상태로 삽입한다.

**등록 패턴:**

등록 순서: `ClearAll() → Register() × N → Build()`. `Register(이름, 설명, ...)`의 이름·설명은 **영어로 작성**한다(CheatConsole UI 표기 규칙).

로컬 초기화 로직은 `ResetLocal(string key)` private 메서드 하나로 묶어 4개 버튼(`Reset Local (DEV)`·`Reset Local+Server (DEV)`·`Reset Local (LIVE)`·`Reset Local+Server (LIVE)`)이 공유한다. 이 메서드 맨 앞에서 `Time.timeScale = 0f;`로 게임을 즉시 정지시킨다 — 앱 재시작 전까지 자동 저장 등 다른 로직이 이어서 돌며 방금 지운 데이터를 다시 써버리는 것을 막는다.
...
- 7-0 치트 "Reset Local (DEV)" 실행 후 앱 재시작 → 서버 데이터로 불러오는지 확인
- 7-0 치트 "Reset Local+Server (DEV)" 실행 후 앱 재시작 → 로컬 데이터로 불러오는지 확인
- 7-0 치트 "Reset Local+Server (LIVE)" 실행 후 실제 서버 데이터가 그대로인지(초기화 안 됨) 확인
