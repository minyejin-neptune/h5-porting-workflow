...
```bash
grep 'WEBGL_\|UNITY_WEBGL' $H5PW_ROOT/templates/porter-rule.md
```
...
> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/platform-patterns.md` → "2. 서버 시간 체크"
...
> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/platform-patterns.md` → "3. 로그인 API 연동"
...
삽입 전 **그 위치의 기존 전처리문 계층을 먼저 확인**한다(`porter-rule.md` § 코딩 컨벤션 — 패턴 A/B, 기존 iOS/Android 분기 주의). 이미 `#if UNITY_WEBGL { ... }` 구조가 있으면 그 안에 위 3조건을 자연스럽게 끼워 넣고, 없으면 플랫 `&&`로 써도 무방하다 — 아래 코드 패턴은 여러 형태 중 하나의 예시일 뿐, 그대로 베끼는 게 목적이 아니다.

> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/platform-patterns.md` → "3-A. 로그인 로그 삽입"
...
> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/platform-patterns.md` → "4. 백그라운드 사운드 처리"
...
> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/platform-patterns.md` → "5-2. 보상형 / 전면 광고 API"
...
> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/platform-patterns.md` → "5-3. 광고 중 BGM / 게임 중지 처리" (A 패턴 · B 패턴 1단계 · 패턴 Y 코드가 모두 이 섹션에 있다)

**1단계 — 광고 매니저 클래스 수정** (B 패턴)
...
`Time.timeScale = 0`에 자동으로 멈추므로 1단계만으로 충분 — 추가 코드 없음.

---

**패턴 Y — `WaitForSecondsRealtime` / `unscaledDeltaTime` / `Stopwatch` 기반 타이머가 있는 경우**

`Time.timeScale`을 무시하는 타이머라 자동으로 멈추지 않는다 — `OnAdVisibilityChangedEvent`를 구독해 직접 정지·재개한다.

> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/platform-patterns.md` → "5-3. 광고 중 BGM / 게임 중지 처리" § 패턴 Y
...
> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/platform-patterns.md` → "6-1. 가격 가져오기"
...
> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/platform-patterns.md` → "6-2. 가격 UI 표시"
...
> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/platform-patterns.md` → "6-3. 구매 연동"
...
> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/platform-patterns.md` → "7-0. 치트 — 서버/로컬 초기화"
...
**2단계 — 암호화 여부 확인 (pureweb-porter가 이미 제거했어야 함 — 여기선 확인용)**

`{LOCAL_SAVE_METHOD}`(= `{SAVE_METHOD}`인 경우 포함)는 이 포터가 재사용하는 공유 함수라, pureweb-porter 8번 스텝에서 이미 암호화를 제거했어야 한다(공유 함수라 늦게 지우면 다른 WebGL 플랫폼도 영향받으므로 pureweb-porter가 먼저 처리하는 게 원칙). 아래로 재확인만 한다:
...
> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/platform-patterns.md` → "7. 서버 저장 / 불러오기 — HLSDK 연동"
...
> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/platform-patterns.md` → "10-2. 랭킹 보드 연동"
...
> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/platform-patterns.md` → "10-3. 랭킹 점수 등록 코드"
