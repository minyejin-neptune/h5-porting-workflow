...
사람 준비 항목(배너 광고 위치·IAP PID 매핑·햅틱 타입·공유하기 문구·프로모션 ID·랭킹 버튼 추가 위치)은 h5-port STEP 3-A가 포터 실행 전에 사용자에게 수집해 포팅 이슈 `## 사람 준비 항목`에 체크리스트로 기록한다 (`[x]` + 확정값 = 확정, `[ ]` = 미확정, 줄 없음 = 해당 없음). 각 step 도달 시 이슈에서 해당 항목을 읽어 처리한다:
...
- 줄 없음(해당 없음) / 포팅 이슈 번호 없음 → 항목이 필요해지면 미확정과 동일하게 처리
...
grep -rn "GenerateHapticFeedback" {SCRIPTS_PATH} --include="*.cs" | grep -v HyperLane
```
...
3. 제안대로 코드를 삽입하고, `toss-checklist.md` `## 확인 필요`에 위 제안 표를 기록해 사람이 나중에 타입·위치를 검토할 수 있게 한다(진동 타입은 UX 품질 문제로 이후 조정 가능).
