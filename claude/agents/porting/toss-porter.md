...
[계획] 기존 WEBGL 분기 현황 파악 + Toss 연동 기능 존재 여부(2-B) 파악
       ...
       → 사람 준비 항목(배너 위치·PID·햅틱·공유 문구 등)은 toss-checklist.md `## 확인 필요`의 `[사람 준비]` 태그 항목 참조 (h5-port가 사전 수집)
      ↓
...
특정 단계만 요청받으면(예: "6-1만 해줘") 이 표에서 선행 단계를 확인해 미완료면 함께 범위에 포함한다. 표에 없으면 선행 조건 없음. VOCAB 조건부 스킵(예: "가격 UI 없으면 스킵")은 선행 단계가 아니라 조건부 실행이므로 이 표에 넣지 않는다 — 각 단계 본문의 조건을 그대로 따른다.

| 단계 | 선행 필요 단계 | 이유 |
|---|---|---|
| 3-A. 로그인 로그 삽입 | 3 | 로그인 완료 흐름(`QuickLogin`→로비 진입) 안에 로그 삽입 지점이 있음 |
| 5-1. 배너 광고 — SafeArea 반영 부분만 | 9 | 배너 높이를 9(SafeArea) 처리 클래스의 bottom padding에 반영. 배너 표시/Attach/Detach 자체는 선행 불필요 |
| 6-1. 가격 가져오기 | 3 | 삽입 위치가 `InitToss()`(3에서 생성) 안, QuickLogin 완료 직후 |
| 6-2. 가격 UI 표시 | 6-1 | `GetProducts()`(6-1)의 로컬 캐싱 결과에 의존 |
| 6-4. 구매 후 유저 데이터 저장 | 6-3 | 6-3의 구매 성공 콜백 구조 안에 저장 호출을 삽입 |
| 6-5. DEV 뒤로가기 처리 | 6-3 | 6-3에 있는 `WEBGL_DEV_VER` 분기를 확인·재사용 |
...
**표에 없는 모든 단계**(2, 4, 5-2, 5-3, 6-3, 7-0, 8, 9, 10-1, 10-3, 10-4, 11~16)는 선행 조건 없음 — 단독 요청 시 바로 실행 가능. 단 5-2/5-3은 서로 코드가 얽혀 있어(5-2가 5-3에서 정의하는 콜백을 미리 호출) 실무적으로는 함께 처리하는 게 안전하다(강제 선행조건은 아님).
...
게이트 실패 시 미충족 항목 목록을 `toss-checklist.md` `## 확인 필요`와 포팅 이슈 `## 확인 필요 / 미확정`에 기록하고, 아래 안내를 출력한 뒤 종료한다 (추론 금지 원칙 — 불완전한 VOCAB 위에서 grep 추측으로 진행하지 않는다):
...
```bash
echo "=== 랭킹 ===" && \
  grep -rln "Leaderboard\|LeaderBoard\|SubmitScore\|RankButton" {SCRIPTS_PATH} --include="*.cs" 2>/dev/null | grep -v HyperLane
echo "=== 공유하기 ===" && \
  grep -rln "NativeShare\|ShareLink\|OnClickShare" {SCRIPTS_PATH} --include="*.cs" 2>/dev/null | grep -v HyperLane
echo "=== 프로모션 ===" && \
...
echo "=== 햅틱 ===" && \
  grep -rln "Vibrate\|Haptic\|haptic\|vibrate" {SCRIPTS_PATH} --include="*.cs" 2>/dev/null | grep -v HyperLane
echo "=== 가격UI ===" && \
  grep -rln "price\|Price\|productPrice\|priceText\|PriceText\|costText" {SCRIPTS_PATH} --include="*.cs" 2>/dev/null | grep -v HyperLane
```
...
prompt에 특정 단계가 명시됐으면(예: "6-1만", "5. 광고 SDK 처리해줘") 그 단계를 범위로 잡는다. 위 "선행 조건 표"에서 그 단계의 선행 단계를 확인해 `## 단계 진행`에서 미완료(`- [ ]`)면 범위에 함께 포함한다(자동으로 먼저 처리). 선행 단계가 이미 `[x]`면 요청받은 단계만 진행한다. 명시되지 않은 다른 단계는 이번 실행에서 건드리지 않는다.

prompt에 특정 단계 명시가 없으면(예: "toss 포팅 해줘", "이어서 해줘") `## 단계 진행`에서 미완료(`- [ ]`)인 단계 전체를 범위로 잡는다 — 이미 `[x]`인 단계는 재실행하지 않는다. 체크리스트 파일 자체가 없으면(최초 실행) 전체 단계가 범위.
...
2-C에서 정한 범위 내 단계만 대상으로 아래 형식의 테이블을 출력한다. grep+VOCAB 근거로 도출된 계획이라 확인 없이 바로 작업을 시작하고, 표는 체크리스트 `## 확인 필요`에도 남겨 사람이 나중에 검토할 수 있게 한다(계획과 다르면 각 단계 진행 중 발견 시 정정):
...
사람 준비 항목(배너 광고 위치·IAP PID 매핑·햅틱 타입·공유하기 문구·프로모션 ID·랭킹 버튼 추가 위치)은 h5-port STEP 3-A가 포터 실행 전에 사용자에게 수집해 `toss-checklist.md` `## 확인 필요`에 `[사람 준비]` 태그로 기록한다 (`- [x] [사람 준비] {항목}: {확정값}` = 확정, `- [ ] [사람 준비] {항목}: 미확정` = 미확정, 줄 없음 = 해당 없음 또는 아직 미수집). 각 step 도달 시 체크리스트에서 해당 항목을 읽어 처리한다:

- `[x]` 확정값 → 그대로 사용 — 재질문 없이 진행
- `[ ]` 미확정 — 공유하기 문구 → 플레이스홀더 삽입 + `// TODO: 공유 문구 기획 확인 필요` (step 11 기존 폴백)
- `[ ]` 미확정 — 햅틱 타입 → 코드 근거로 제안 타입 삽입 + toss-checklist `## 확인 필요`에 검토 기록 (step 8 기존 폴백)
- `[ ]` 미확정 — 배너 위치·IAP PID 매핑·프로모션 ID·랭킹 버튼 위치 → 그 값이 필요한 세부 작업만 스킵 + toss-checklist `## 확인 필요` 기록 (나머지 작업은 진행)
...
**HLSDK 없음 →** 대체 SDK 탐색 결과가 없으므로(탐색 기본 원칙 4번) `toss-checklist.md` `## 확인 필요`에 "HLSDK 없음 — 광고 중 BGM 차단 처리할 대체 SDK 경로 확인 필요" 기록하고 이 단계는 스킵한다.

**HLSDK 있음 →** 단계 2로 진행.

**단계 2 — HLSDK ShowAd 콜백 내부 AudioListener 처리 여부 확인**

CLAUDE.md 규칙상 HyperLane은 사전 허락 없이 읽지 않는다. 서브에이전트는 실시간으로 허락을 물어볼 수 없으므로 허락을 받지 못한 것으로 간주하고 아래를 그대로 적용한다: **8-3을 그대로 진행하되, 완료 후 테스트 항목에 "광고 중 BGM이 두 번 멈추지 않는지" 확인을 추가한다.**
...
- **`InitToss()`의 성공 여부를 호출부(`{GAME_INIT_METHOD}`)도 확인해야 한다** — `InitToss()` 내부의 `yield break`/`return`은 `InitToss()` 자신만 멈춘다. `StartCoroutine(InitToss())`/`await InitToss()`로 호출한 쪽은 결과를 모른 채 그대로 다음 로직으로 넘어가므로, 로그인 결과를 필드(Coroutine) 또는 반환값(UniTask, `UniTask<bool>`)으로 전달받아 호출부에서도 `if (!success) yield break/return`을 넣는다 (아래 예시 참조)
...
- **로그인 후 작업이 많을 경우 — private 함수 분리**: 로그인 이후 데이터 로드·SDK 초기화 등 처리가 길어지면 코루틴 본체에서 private 메서드로 분리해 가독성을 유지한다.
  ...
  **Coroutine 패턴:**
  ...
  private IEnumerator {GAME_INIT_METHOD}()
  {
      ...
      if (!_tossLoginSuccess) yield break; // 로그인 실패 — 이후 데이터 로드 등 기존 로직 진행 금지
  #endif
  #endif
      // 기존 로직 계속
  }
  ...
  #if UNITY_WEBGL && WEBGL_TOSS
  private bool _tossLoginSuccess;
  ...
  private IEnumerator InitToss()
  {
      ...
      HLSDK.Instance.QuickLogin(loginOk => loginResult = loginOk);
      yield return new WaitUntil(() => loginResult.HasValue);
      _tossLoginSuccess = loginResult == true;
      if (!_tossLoginSuccess) yield break;
  ...
  **UniTask 패턴:**
  ...
  #endif
  #if WEBGL_TOSS
      bool tossLoginSuccess = await InitToss();
      if (!tossLoginSuccess) return; // 로그인 실패 — 이후 데이터 로드 등 기존 로직 진행 금지
  #endif
  #endif
      // 기존 로직 계속
  }
  ...
  #if UNITY_WEBGL && WEBGL_TOSS
  private async UniTask<bool> InitToss()
  {
      bool? loginResult = null;
      HLSDK.Instance.QuickLogin(loginOk => loginResult = loginOk);
...
- 없음 → 9번 SafeArea 처리 클래스에 배너 높이(`HLSDK.Instance.GetBannerHeight()` 등)를 bottom padding에 추가로 반영한다 (대안이 없는 기계적 처리 — 확인 없이 적용, 결과는 아래 인수 테스트로 확인)
...
> toss-checklist.md `## 확인 필요`의 `[사람 준비]` 태그 항목 중 **IAP PID 매핑** 값을 먼저 확인한다 — 미체크(`[ ]` 미확정)이면 계획 단계의 미확정 처리 목록을 따른다.
...
**완료 신호**: `{GAME_INIT_METHOD}`(또는 `InitToss()`) 안에 `HLSDK.Instance.GetProducts(` 호출 이미 존재 → 스킵.
...
| 가격 하드코딩 | 상수·고정 문자열로 설정됨 | → 결정 필요 라우팅(하드코딩 가격의 Toss 실가격 교체 여부 — 실가격 미확정 가능성). 교체 보류, 해당 위치에 `// TODO: Toss 실가격 교체 확인 필요` 표시 |
...
**완료 신호**: 6-3의 `PurchaseByOriginalPID` 실패 콜백 안에 `#if WEBGL_DEV_VER` 분기 이미 존재 → 스킵.

```bash
grep -rn "WEBGL_DEV_VER" {SCRIPTS_PATH} --include="*.cs" | grep -v HyperLane | grep -i "purchase\|iap\|buy"
```

DEV 분기에서 결제창 뒤로가기 시 강제 지급하는 코드가 없으면 위 6-3 패턴의 `WEBGL_DEV_VER` 분기 확인.
...
- 암호화 없음 → 3단계로
- **암호화 있음 → 제거 (고정 정책 — 대안 없음)**: SetUserData는 평문(또는 Base64)만 허용하므로 확인 없이 암호화 로직을 `#if !UNITY_WEBGL`로 격리하고 WebGL 분기에서 제거한다. 보안 관련 변경이므로 알림을 반드시 남긴다 — 포팅 이슈 `## 확인 필요 / 미확정`에 `- [x] 🔐 암호화 제거됨 — {발견 메서드명}, WebGL 분기는 Base64만 사용` 기록(체크된 상태 — close를 막지 않는 알림) + toss-checklist `## 교정 기록`에도 기록.
...
> toss-checklist.md `## 확인 필요`의 `[사람 준비]` 태그 항목 중 **햅틱 타입** 값을 먼저 확인한다 — 미체크(`[ ]` 미확정)이면 계획 단계의 미확정 처리 목록을 따른다(제안 삽입+검토 기록).
...
3. 제안대로 코드를 삽입하고, `toss-checklist.md` `## 확인 필요`에 위 제안 표를 기록해 사람이 나중에 Tier·위치를 검토할 수 있게 한다(진동 세기는 UX 품질 문제로 이후 조정 가능).
...
> toss-checklist.md `## 확인 필요`의 `[사람 준비]` 태그 항목 중 **랭킹 버튼 위치** 값을 먼저 확인한다 — 미체크(`[ ]` 미확정)이면 계획 단계의 미확정 처리 목록을 따른다.
...
- 신규 추가 필요(0건) → toss-checklist.md `## 확인 필요`의 `[사람 준비]` 태그 항목 중 **랭킹 버튼 위치** 값을 확인한다. 확정값 있으면 그 위치에 구현(기존 UI 관리 클래스에 삽입 또는 별도 버튼 오브젝트 + 클릭 핸들러), 미체크(미확정)이면 → 결정 필요 라우팅(랭킹 버튼 추가 위치). 확정 전까지 버튼 신규 추가는 스킵.
...
> toss-checklist.md `## 확인 필요`의 `[사람 준비]` 태그 항목 중 **공유하기 문구** 값을 먼저 확인한다 — 미체크(`[ ]` 미확정)이면 아래 기존 폴백(플레이스홀더+TODO)을 그대로 적용한다.
...
클릭 핸들러를 Read해서 함수 본문 확인. 공유 문구 확정 여부를 실시간으로 물어볼 수 없으므로 플레이스홀더로 삽입 후 `// TODO: 공유 문구 기획 확인 필요` 주석을 추가하고, `toss-checklist.md` `## 확인 필요`에도 기록한다.
...
공유 문구는 toss-checklist.md `## 확인 필요`의 `[사람 준비]` 태그 항목 중 **공유하기 문구** 값을 따른다 — 미체크(미확정)이면 플레이스홀더로 삽입 후 `// TODO: 공유 문구 기획 확인 필요` 주석 추가.
...
> - 기존 표시 UI가 없음 → 어느 화면에 추가할지는 사람 판단이 필요하므로 `toss-checklist.md` `## 확인 필요`에 "UID/version 표시 위치 확인 필요" 기록 후 이 단계 스킵
...
> - 미구현 → `toss-checklist.md` `## 확인 필요`에 "로컬라이제이션 미구현 — 범위 결정 후 별도 작업" 기록
...
이 단계는 이미 👤(사람 결정) — 위 현황을 `toss-checklist.md` `## 확인 필요`에 기록만 하고 실행은 사람이 결정할 때까지 하지 않는다.
