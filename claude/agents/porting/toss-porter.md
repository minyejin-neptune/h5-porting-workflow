...
> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/toss-patterns.md` → "5-1. 배너 광고"
...
> **코드 패턴**: `$H5PW_ROOT/templates/porter-patterns/toss-patterns.md` → "12. 프로모션"
...
| 다건 임계값 기반 (동일 prefix + 숫자, 예: `NORMAL_PLAY_3/7/15`) | 임계값 갱신 지점(예: `TotalPlayCount` 증가 직후) — prefix 파싱 + Dictionary 캐싱 패턴 사용 |
...
- 다건 임계값 기반(동일 prefix + 숫자, 예: `NORMAL_PLAY_3/7/15`)은 개별 하드코딩 대신 prefix 파싱 + `Dictionary` 캐싱 패턴 사용 — 체크 시점마다 `for`문 순회 없이 `TryGetValue`로 O(1) 조회. 코드 패턴: `toss-patterns.md` → "다건 임계값 기반 프로모션 패턴 (prefix + Dictionary)"
