...
> 1. 포팅 이슈(진입점 **0-C단계 포팅 이슈 확보**에서 확정한 번호)의 `## 확인 필요 / 미확정`에 `- [ ] {결정 항목} — {선택지·판단 맥락}` 추가 (`gh issue edit`). 이슈를 못 확보한 경우(NO_REMOTE)에만 생략.
> 2. `toss-checklist.md` `## 확인 필요`에 같은 항목 기록.
...
> - ❓ **판단 필요** — AI가 탐색 후 결정 필요 라우팅(포팅 이슈 기록)으로 사람 결정 대기
...
[계획] 기존 WEBGL 분기 현황 파악 + Toss 연동 기능 존재 여부(2-B) 파악
       ...
       → 사람 준비 항목(배너 위치·PID·햅틱·공유 문구 등)은 포팅 이슈 `## 사람 준비 항목` 참조 (h5-port가 사전 수집)
      ↓
...
3. 없음 → `~/github/h5-porting-workflow/templates/porting-issue-body.md`를 Read해 제목·본문 형식대로 직접 생성한다 (`gh issue create`). 직접 생성하는 이슈에는 h5-port STEP 3-A 답변이 없으므로 `## 사람 준비 항목`을 전부 `[ ]` 미확정으로 생성한다 — 각 step의 기존 미확정 폴백이 그대로 적용되고, h5-port 후속 모드가 나중에 수집한다. 생성 실패 시 에러 원문을 완료 리포트에 포함하고 이슈 없이 진행한다.
...
아래 형식으로 테이블을 출력한다. grep+VOCAB 근거로 도출된 계획이라 확인 없이 바로 작업을 시작하고, 표는 체크리스트 `## 확인 필요`에도 남겨 사람이 나중에 검토할 수 있게 한다(계획과 다르면 각 단계 진행 중 발견 시 정정):
...
사람 준비 항목(배너 광고 위치·IAP PID 매핑·햅틱 타입·공유하기 문구·프로모션 ID·랭킹 버튼 추가 위치)은 h5-port STEP 3-A가 포터 실행 전에 사용자에게 수집해 포팅 이슈 `## 사람 준비 항목`에 체크리스트로 기록한다 (`[x]` + 확정값 = 확정, `[ ]` = 미확정, 줄 없음 = 해당 없음). 0-C단계에서 포터가 이슈를 직접 생성한 경우 이 섹션은 전부 `[ ]` 미확정 상태다. 각 step 도달 시 이슈에서 해당 항목을 읽어 처리한다:
...
- 줄 없음(해당 없음) / 이슈를 못 확보(NO_REMOTE) → 항목이 필요해지면 미확정과 동일하게 처리
...
> 포팅 이슈 `## 사람 준비 항목`의 **배너 위치** 값을 먼저 확인한다 — 미체크(`[ ]` 미확정)이면 계획 단계의 미확정 처리 목록을 따른다.
...
- 없음 → 12번 SafeArea 처리 클래스에 배너 높이(`HLSDK.Instance.GetBannerHeight()` 등)를 bottom padding에 추가로 반영한다 (대안이 없는 기계적 처리 — 확인 없이 적용, 결과는 아래 인수 테스트로 확인)
...
> 포팅 이슈 `## 사람 준비 항목`의 **IAP PID 매핑** 값을 먼저 확인한다 — 미체크(`[ ]` 미확정)이면 계획 단계의 미확정 처리 목록을 따른다.
...
> 포팅 이슈 `## 사람 준비 항목`의 **햅틱 타입** 값을 먼저 확인한다 — 미체크(`[ ]` 미확정)이면 계획 단계의 미확정 처리 목록을 따른다(제안 삽입+검토 기록).
...
> 포팅 이슈 `## 사람 준비 항목`의 **랭킹 버튼 위치** 값을 먼저 확인한다 — 미체크(`[ ]` 미확정)이면 계획 단계의 미확정 처리 목록을 따른다.
...
- 신규 추가 필요(0건) → 포팅 이슈 `## 사람 준비 항목`의 **랭킹 버튼 위치** 값을 확인한다. 확정값 있으면 그 위치에 구현(기존 UI 관리 클래스에 삽입 또는 별도 버튼 오브젝트 + 클릭 핸들러), 미체크(미확정)이면 → 결정 필요 라우팅(랭킹 버튼 추가 위치). 확정 전까지 버튼 신규 추가는 스킵.
...
> 포팅 이슈 `## 사람 준비 항목`의 **공유하기 문구** 값을 먼저 확인한다 — 미체크(`[ ]` 미확정)이면 아래 기존 폴백(플레이스홀더+TODO)을 그대로 적용한다.
...
공유 문구는 포팅 이슈 `## 사람 준비 항목`의 **공유하기 문구** 값을 따른다 — 미체크(미확정)이면 플레이스홀더로 삽입 후 `// TODO: 공유 문구 기획 확인 필요` 주석 추가.
...
> 포팅 이슈 `## 사람 준비 항목`의 **프로모션 ID** 값을 먼저 확인한다 — 미체크(`[ ]` 미확정)이면 계획 단계의 미확정 처리 목록을 따른다.
