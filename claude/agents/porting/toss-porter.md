...
사람 준비 항목(배너 광고 위치·IAP PID 매핑·햅틱 타입·공유하기 문구·프로모션 ID·랭킹 버튼 추가 위치)은 h5-port STEP 3-A가 포터 실행 전에 사용자에게 수집해 포팅 이슈 `## 사람 준비 항목`에 기록한다. 각 step 도달 시 이슈에서 해당 값을 읽어 처리한다:

| 값 | 처리 |
|---|---|
| 확정값 | 그대로 사용 — 재질문 없이 진행 |
...
| 해당 없음 / 포팅 이슈 번호 없음 | 항목이 필요해지면 미확정과 동일하게 처리 |
...
> 포팅 이슈 `## 사람 준비 항목`의 **배너 위치** 값을 먼저 확인한다 — `⏸ 미확정`이면 계획 단계의 미확정 처리 표를 따른다.
...
> 포팅 이슈 `## 사람 준비 항목`의 **IAP PID 매핑** 값을 먼저 확인한다 — `⏸ 미확정`이면 계획 단계의 미확정 처리 표를 따른다.
...
> 포팅 이슈 `## 사람 준비 항목`의 **햅틱 타입** 값을 먼저 확인한다 — `⏸ 미확정`이면 계획 단계의 미확정 처리 표를 따른다(제안 삽입+검토 기록).
...
> 포팅 이슈 `## 사람 준비 항목`의 **랭킹 버튼 위치** 값을 먼저 확인한다 — `⏸ 미확정`이면 계획 단계의 미확정 처리 표를 따른다.
...
> 포팅 이슈 `## 사람 준비 항목`의 **공유하기 문구** 값을 먼저 확인한다 — `⏸ 미확정`이면 아래 기존 폴백(플레이스홀더+TODO)을 그대로 적용한다.
...
> 포팅 이슈 `## 사람 준비 항목`의 **프로모션 ID** 값을 먼저 확인한다 — `⏸ 미확정`이면 계획 단계의 미확정 처리 표를 따른다.
