...
[계획] 기존 WEBGL 분기 현황 파악 + Toss 연동 기능 존재 여부 파악
      ...
      ↓
[병렬 가능] 5-1 배너 | 6-5 DEV 뒤로가기 | 12 프로모션 | 14 불필요 UI 삭제 | 15 로컬라이제이션(확인 필요) | 16 용량 최적화
      ↓
...
**표에 없는 모든 단계**(12, 14, 15, 16)는 선행 조건 없음 — 단독 요청 시 바로 실행 가능. 단, 이 파일의 모든 단계는 **platform-porter 선행 완료**가 공통 전제다(아래 진입점 게이트 참조).
...
| [2] 프로모션 | ✅ | ⚠️ 분기 누락 — 재처리 |
| [3] 불필요한 UI | ✅ | ⚠️ 가드 누락 — 재처리 |
| [4] 전처리문 WEBGL_TOSS | ✅ | ⚠️ UNITY_WEBGL 누락 — 재처리 |
