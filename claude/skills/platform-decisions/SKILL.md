...
> **공용 규칙**: `~/github/h5-porting-workflow/templates/porter-rule.md`를 Read해서 따른다(탐색 기본 원칙·문서 오류 교정 기록·에디터 섀도잉 금지·전처리문 규칙 등) — 이 경로는 저장소 루트 기준이다(스킬 자기 디렉토리의 `references/`와는 별개). 이 스킬은 platform-porter와 동일한 `platform-checklist.md`를 정본으로 쓴다. `{PLATFORM_SYMBOL}`은 `.porting-context`로 현재 선택된 플랫폼을 읽어 사용(예: 없으면 `UNITY_WEBGL` 단독으로 충분 — porter-rule.md 참조).
