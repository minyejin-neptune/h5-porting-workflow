...
> **STEP 번호는 실행/문서 순서가 아니라 고정 식별자다.** 아래 본문은 위 다이어그램대로 병렬 그룹(1·3·6·4)을 먼저, 순차 그룹(D→A 교차검증·2·5)을 뒤에 제시하므로 번호가 뒤섞여 보인다. 번호를 재정렬하지 않는 이유는 h5-port·stats-logging이 `STEP 3`·`STEP 4-C` 등으로 교차참조하기 때문이다 — 실행 순서는 항상 이 다이어그램을 기준으로 한다.
...
Coroutine 기반 타이머 히트 파일 목록도 함께 기록해둔다 (확정 시 포터에 전달).

scan은 여기까지다 — `게임중지: 확인 필요`로 남기고 종료한다. 확정값(`게임중지: 필요` / `게임중지: 불필요`)은 scan 자신(완료 후 채팅 출력)이 아니라 **h5-port STEP 2-B(분석 산출물 커밋 직후)**가 위 탐색 근거를 제시해 사용자에게 묻고 VOCAB에 기록한다. scan을 단독 실행한 경우에는 사용자가 직접 확정할 때까지 `확인 필요`로 남는다.
...
> **0건 처리 (4-I 공통 — 거짓 음성 방지)**: 아래 각 항목의 1차 grep이 0건이어도 곧바로 `없음`으로 단정하지 않는다 — 좁은 키워드 grep은 자체 네이밍·한글 클래스명을 놓칠 수 있다. 0건이면 그 항목의 도메인 어휘(한글·동의어 포함)로 1회 더 넓게 재탐색한다(최대 5개 파일, 1단계). 넓힌 결과도 0건이면 `없음`(확신), 후보는 나왔으나 Read로 확정 못 하면 `확인 필요`로 기록한다. 아래 각 항목의 "없으면 없음"은 이 절차를 거친 뒤의 최종 판정을 뜻한다. (햅틱은 예외 — 0건이면 `역기획 필요`.)
