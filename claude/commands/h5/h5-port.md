...
- 배너 위치: {상단 | 하단 | ⏸ 미확정 | 해당 없음}
- IAP PID 매핑: {확정 내용 | ⏸ 미확정 | 해당 없음}
- 햅틱 타입: {확정 내용 | ⏸ 미확정 | 해당 없음}
- 공유하기 문구: {확정 문구 | ⏸ 미확정 | 해당 없음}
- 프로모션 ID: {ID | ⏸ 미확정 | 해당 없음}
- 랭킹 버튼 위치: {위치 | ⏸ 미확정 | 해당 없음}
...
포터를 실행하기 직전, 포팅 이슈 `## 사람 준비 항목`에 `⏸ 미확정`이 남아 있으면 AskUserQuestion으로 한 번 재확인한다 — "그 사이 확정된 항목이 있나요?" (질문 수집과 포터 실행 사이에 기획이 확정됐을 수 있다). 확정으로 바뀐 답은 이슈 본문을 갱신한 뒤 진행한다.
...
STEP 3-B에서 이슈를 생성했으면(없으면 이 단계 생략):
