...
hook 스크립트는 `install.sh`가 이미 `~/.claude/hooks/compile-check.sh`로 심볼릭 링크해뒀다(대상: `$H5PW_ROOT/templates/scripts/compile-check.sh`). settings.json에는 **이 고정 경로**를 쓴다 — repo 절대경로를 박아넣으면 repo를 옮겼을 때 깨지지만, 심볼릭 경로는 `install.sh` 재실행만으로 갱신된다.

먼저 링크와 jq를 확인한다. 링크가 없으면 `install.sh`를 다시 실행하도록 안내한다:

```bash
[ -e ~/.claude/hooks/compile-check.sh ] && echo "LINK_OK" || echo "LINK_MISSING — repo에서 ./install.sh 재실행 필요"
...
인자를 넘기지 않는다 — 인자 없이 호출되면 hook 모드로 동작해 stdin JSON에서 `.cs` 여부를 판별하고 플랫폼은 `.porting-context`에서 읽는다(hook은 인자를 받을 수 없기 때문).
...
### STEP 3-A. 사람 준비 항목 확인

포터는 서브에이전트라 실행 중 사용자에게 물어볼 수 없다 — 사람이 결정·준비해야 하는 항목을 포터 실행 전에 여기서 수집한다.

**퓨어웹 포함 모든 포팅 — 사전 빌드 용량 baseline 측정 여부**: pureweb-porter의 "사전 빌드 용량 기록"은 시간이 걸리는 빌드다. 포터가 임의로 스킵하지 않도록, 여기서 AskUserQuestion으로 측정 여부를 정한다(선택지: `측정함` / `생략`(기본)). 답을 `pureweb-checklist.md` `## 확인 필요`에 `- [x] [사람 준비] 사전 빌드 baseline 측정: 측정함` 또는 `생략`으로 기록한다(파일 없으면 최소 헤더로 생성). pureweb-porter는 이 값을 읽어 따른다.

아래 항목들은 **토스 포팅인 경우에만** 추가로 확인한다(퓨어웹 단독이면 baseline 항목만 확인하고 넘어간다).
...
> 햅틱 타입·공유하기 문구·랭킹 버튼 추가 위치는 이 표에서 제외됐다 — platform-porter가 해당 스텝을 실행하지 않고 `/platform-decisions` 스킬로 이관하므로(이슈 #48), 여기서 미리 물어볼 필요가 없다. STEP 4 완료 후 안내를 참조.
...
### STEP 3-B. 포팅 이슈 생성 (게임 repo에 gh remote 있는 경우만 — 스텝별)

```bash
gh repo view --json nameWithOwner -q .nameWithOwner 2>/dev/null && echo "REPO_OK" || echo "NO_REMOTE"
```

- `NO_REMOTE` → 이슈 생성 생략, STEP 3-C(포터 실행)로 바로 진행 (STEP 3-A 답변은 이미 STEP 3-A에서 체크리스트에 직접 기록했으므로 이슈 없어도 문제 없음).
- `REPO_OK` → 이번에 실행할 포터마다(pureweb·platform은 항상, 토스 포팅이면 toss도) **스텝 1개당 이슈 1개**를 생성한다.

**대상 스텝 목록 확보**: 해당 포터의 체크리스트 파일(`{platform}-checklist.md`)이 이미 있으면 그 파일 `## 단계 진행`의 미완료(`- [ ]`) 항목만 대상으로 삼는다. 파일이 없으면(최초 실행) 그 포터 파일의 "## 체크리스트 관리" `### 파일 초기 형식`에 있는 기본 스텝 목록 전체가 대상이다 — 단계 목록은 그 포터 파일이 유일한 소스, 여기서 재정의하지 않는다.
...
- 있으면 → 그 번호를 재사용하고 생성을 생략한다(중복 생성 방지).
- 없으면 → `/common:create-issue --no-confirm`으로 생성한다(실패·확인 처리는 그 스킬이 담당). 이슈 내용:
  - 제목: `[포팅] {PLATFORM} {STEP_ID} — {스텝명}` (예: `[포팅] PUREWEB 8-A — 저장 키 분리`)
  - DoD 체크박스 1개: `- [ ] {스텝명} 완료 (완료 신호·체크리스트 업데이트 규칙은 {PLATFORM}-porter.md 참조)`
  - 참고: "정본은 `{PLATFORM}-checklist.md` `## 단계 진행`의 해당 줄이다. 이 이슈는 그 줄을 비추는 미러일 뿐이다."

생성·재사용한 전체 결과를 **스텝ID:이슈번호 매핑**으로 모은다 — 표기 형식: `{STEP_ID}=#{이슈번호}`를 쉼표로 나열(예: `0=#48, 1=#49, 1-A=#50, ...`). 포터별로 매핑을 나눠 기억해 STEP 3-C에서 해당 포터의 prompt에만 그 포터 몫을 전달한다.
...
포팅 이슈 매핑: {STEP 3-B에서 이 포터 몫으로 생성한 스텝ID:이슈번호 목록, 예: 0=#48, 1=#49, 1-A=#50 — 없으면 "없음"}
```

> "요청 범위"(특정 단계만) 필드는 넣지 않는다 — h5-port는 항상 전체 파이프라인 실행을 원하므로, 단계 미지정 시 포터의 "실행 범위 결정"이 기본값으로 잡는 "미완료 단계 전체"와 정확히 일치한다. 특정 단계만 요청하는 시나리오는 사용자가 포터를 직접 호출할 때만 발생한다.
...
### 검증 기록 교차 확인 (포터 자기보고를 신뢰하지 않음, #54)

관련된 각 체크리스트(`pureweb-checklist.md`·`platform-checklist.md`·토스 포팅이면 `toss-checklist.md`)를 Read해서 `## 단계 진행`의 `검증` 행이 실제 결과로 채워져 있는지 확인한다(`porter-rule.md` § 검증 결과 기록 참조 — 포터가 자기 검증 완료 후 직접 기록해야 하는 줄이다).

- `- [x] 검증 — ✅ ...` 형식으로 채워져 있으면 → 포터가 자기 검증을 실행했다는 신호. 정상 진행.
- `- [ ] 검증`으로 비어 있으면(또는 결과 없이 체크만 돼 있으면) → 포터가 자기 검증 절차를 건너뛰었을 가능성이 있다. 위 STEP 4 자체 검증이 통과했더라도 그것과 무관하게 사용자에게 명시적으로 알린다: `⚠️ {checklist-file}에 검증 기록 없음 — 포터가 자기 검증을 건너뛰었을 수 있습니다.`

### STEP 4-A. 포팅 이슈 종료 처리 (스텝별)

STEP 3-B에서 이슈를 생성했으면(없으면 이 단계 생략). 후속 모드(부분 수정 재호출)에서도 동일하게 적용한다:

포터별 체크리스트(`pureweb-checklist.md`·`platform-checklist.md`·(토스 포팅이면) `toss-checklist.md`)의 `## 단계 진행`을 다시 읽어, STEP 3-B의 스텝ID:이슈번호 매핑과 한 줄씩 대조한다:

- 해당 스텝이 `- [x]`(완료)이고 그 스텝에 연결된 `## 확인 필요` 미체크(`- [ ]`) 항목이 없으면 → 그 스텝의 이슈만 `gh issue close`로 종료(완료 커밋 해시를 코멘트에 포함).
- 해당 스텝이 아직 `- [ ]`(미완료)이거나 `## 확인 필요`에 그 스텝 관련 미체크 항목이 남아 있으면 → 그 스텝의 이슈는 **open으로 유지**하고 남은 사유를 코멘트로 남긴다.
- 검증(STEP 4) 결과가 `❌ 미처리`이면, 미처리로 지목된 스텝의 이슈만 open 유지하고 나머지 완료 스텝 이슈는 위 규칙대로 개별 close한다 — 검증 실패가 전체 이슈를 일괄 묶어두는 근거가 되지 않는다.

체크리스트가 작업 진행의 정본이고, 각 스텝 이슈는 그 줄 하나를 비추는 미러다 — 기반(컴파일/런타임/공백) 스텝 이슈는 `pureweb-checklist.md`, HLSDK 공통 스텝 이슈는 `platform-checklist.md`, 플랫폼 전용 스텝 이슈는 `Docs/porting/{platform}-checklist.md`가 각각 정본이다. 상태가 어긋나면 체크리스트를 기준으로 이슈를 갱신한다.

**최종 완료 보고에 추가**: `platform-checklist.md` `## 단계 진행`에 `/platform-decisions`로 이관된 스텝(8·10-1·11·13·14·15 중 `⏭️ 스킵` 상태인 것)이 있으면, 완료 보고 마지막에 안내한다:

```
남은 판단 항목: {스텝 목록}
→ /platform-decisions (인자 없이 실행하면 대기 항목부터 확인)
```
