...
- Before any code analysis response, ask the user to choose the output format (save as `.md` file / print in chat).
    - If `.md` file is chosen: default location is under `Docs/` (see "Documentation Rules" below for the subfolder by type). Do NOT duplicate output in chat.
...
> 지원 플랫폼이 새로 추가되면(예: 신규 플랫폼 포팅 도입) 이 표에 해당 플랫폼 프리픽스도 함께 추가한다.
