...
- [ ] **H 카테고리 방향 표기 모호성**: `content-relation-mapping-rules.md`의 H-A/H-B 화살표 표기(`다른 시스템→이 콘텐츠` 등)가 "읽는 쪽→읽히는 쪽" 의미로 쓰였는데, 화살표는 통상 "영향이 흘러가는 방향"으로 읽혀 정반대로 오해될 수 있음. 박물관(Museum) 실사용 dry-run에서 서브에이전트가 이 모호함에 실제로 걸려 "확인 필요"로 처리함(추론 금지 원칙은 정상 작동, 표기 자체가 문제). 화살표를 "A가 B를 읽는다" 식 문장으로 교체 필요 — 아직 미수정
